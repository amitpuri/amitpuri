<!--
**amitpuri/amitpuri** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:
-->
[![MIT license](https://img.shields.io/badge/License-MIT-blue.svg)](https://lbesson.mit-license.org/)

## Hello World 👋 

- 🔭 I’m working on my strengths 
- 🌱 I’m learning all the time
- 👯 I’m looking to collaborate on <a href="https://www.linkedin.com/in/amitpuriindia/"><img src="https://img.shields.io/badge/LinkedIn--_.svg?style=social&logo=linkedin" alt="LinkedIn"></a> and <a href="https://github.com/amitpuri"><img src="https://img.shields.io/github/followers/amitpuri.svg?label=GitHub&style=social" alt="GitHub"></a>   
- 💬 Ask me about Microsoft Tech, Cloud and Agile
- 📫 How to reach me: <a href="https://twitter.com/iamamitpuri"><img src="https://img.shields.io/twitter/follow/iamamitpuri?label=Twitter&style=social" alt="Twitter"></a>
- 😄 Pronouns: techies, geek, nerd, :man: 
- ⚡ Fun fact: all false play here :laughing:
- Technophile by instinct,
- Developer by heart,
- Architect by profession, 
- Mentor by passion
- and [amitpuri.com](http://amitpuri.com) my personal website 

![Stats](https://github-readme-stats.vercel.app/api?username=amitpuri&show_icons=true&theme=radical&count_private=true)
![Top-langs](https://github-readme-stats.vercel.app/api/top-langs?username=amitpuri&show_icons=true&theme=radical&count_private=true&hide=html,css,scss)
 
# My Bookmarks of interest
# Stay tuned! it's building up each day...

  * Virtual Hub
    * https://aka.ms/virtualhub
    * https://aka.ms/virtualhub/microsoftteamsand365platform
  * Check out repos 
    * https://github.com/Microsoft
    * https://github.com/SharePoint
    * https://github.com/PNP
    * https://github.com/OfficeDev
    * https://github.com/sdras/awesome-actions
    * https://github.com/awesome-lists/awesome-bash
    * https://github.com/deislabs/akri
    * https://github.com/microsoft/ignite-learning-paths-training-data
  * ML 101
    * https://goo.gl/5Wd2vy
  * Machine Learning by Tom Mitchell
    * http://www.cs.cmu.edu/~tom/10701_sp11/lectures.shtml
  * Interpretable ML book - Explainable AI
    * https://christophm.github.io/interpretable-ml-book
  * Useful AI, ML and Data science resources
    * https://machinelearningmastery.com
    * https://github.com/alicezheng/feature-engineering-book
    * https://towardsdatascience.com/the-7-steps-of-machine-learning-2877d7e5548e
    * https://selfdrivingcars.mit.edu
    * https://hadrienj.github.io/posts/Deep-Learning-Book-Series-2.1-Scalars-Vectors-Matrices-and-Tensors
    * https://becominghuman.ai/building-an-image-classifier-using-deep-learning-in-python-totally-from-a-beginners-perspective-be8dbaf22dd8
    * http://scenesegmentation.csail.mit.edu/
    * https://ai.google
    * https://aischool.microsoft.com
    * https://github.com/amitpuri/data-science-blogs
  * Big O Cheatsheet
    * https://www.bigocheatsheet.com/
  * C# 101
    * https://aka.ms/dotnet3-csharp
  * .NET 
    * https://github.com/dotnet
    * https://github.com/dotnet-presentations
  * .NET Application Architecture - Reference Apps
    * https://github.com/dotnet-architecture
  * Cloud 
    * http://aws.amazon.com/well-architected
    * https://aka.ms/theframework
    * https://www.wellarchitectedlabs.com
    * https://aka.ms/architecture/learn
    * https://aka.ms/architecture/review
  * Azure
    * https://github.com/Azure
    * https://github.com/AzureCAT-GSI
    * https://github.com/AzureCAT-GSI/Bookmarks
    * https://github.com/Azure-Samples
    * https://github.com/Azure-Readiness
    * https://aka.ms/AzureMapsSamples
    * https://github.com/microsoft/Learning-ARM
    * https://olohmann.github.io/azure-hands-on-labs
    * https://azure.github.io/actions/
  * AWS
    * https://github.com/aws-samples/aws-serverless-workshops
      * https://data-processing.serverlessworkshops.io
      * https://event-driven-architecture.workshop.aws
      * https://webapp.serverlessworkshops.io
      * https://async-messaging.workshop.aws
  * AKS Workshop
    * https://aksworkshop.io
  * EKS Workshop
    * https://www.eksworkshop.com
  * Azure Red Hat Openshift Workshop
     * https://aroworkshop.io
  * Openshift on IBM Cloud workshops
     * https://ibm.github.io/openshift-on-ibm-cloud-workshops
  * Istio Service Mesh Workshop
     * https://www.istioworkshop.io
  * Bot Framework   
    * https://github.com/microsoft/BotBuilder-Samples
  * IoT
    * https://github.com/Azure/Industrial-IoT
    * https://github.com/Azure/azure-iot-remote-monitoring
  * A Developer's Introduction to Data Science Video Series
    * https://github.com/microsoft/c9-dev-intro-data-science
  * .NET Foundation
    * https://dotnetfoundation.org
  * .NET
    * http://dot.net
    * https://aka.ms/dotnethelloworld
    * http://dot.net/dotnet/architecture-guides
    * .NET Standard https://dotnet.microsoft.com/platform/dotnet-standard    
    * http://dot.net/videos
  * Microservices with .NET
    * https://dotnet.microsoft.com/apps/aspnet/microservices
    * https://aka.ms/containerfastmode
  * Cloud Native
    * Architecting Cloud Native .NET Applications for Azur https://docs.microsoft.com/dotnet/architecture/cloud-native
    * IBM Garage Cloud Native Bootcamp https://cloudnative101.dev
  * Migrate to Azure App Service
    * https://appmigration.microsoft.com
  * Microsoft Docs 
    * https://docs.microsoft.com 
    * https://learn.microsoft.com
    * https://aka.ms/learn
  * Katacoda
    * https://katacoda.com/courses/kubernetes
  * Open Service Broker for Azure
    * https://osba.sh - This project is no longer being maintained by Microsoft. Please refer to https://github.com/Azure/azure-service-operator for an alternate implementation.
  * Azure Service Operator
    * https://github.com/Azure/azure-service-operator
  * AzOps
    * https://github.com/Azure/AzOps
  * Keda - Kubernetes Event-driven Autoscaling
    * https://keda.sh
  * https://aka.ms/how-bridge-to-k8s-works
  * Akka.NET
    * https://getakka.net
  * Microsoft Azure
    * https://docs.microsoft.com/en-us/learn/azure
    * https://azure.microsoft.com/en-us/updates
    * https://docs.microsoft.com/en-us/azure/architecture
    * https://aka.ms/awsazureguide
    * https://aka.ms/Azure/Shortcuts
    * https://azure.github.io/AppService
    * https://azureweekly.info
    * https://azureworkshops.github.io
    * https://azurecitadel.com
    * https://microsoftcloudworkshop.com
    * https://aka.ms/AzureCompliance
    * https://aka.ms/acr
    * https://aka.ms/migrateapps
    * https://aka.ms/migrateapps/adfssolutionguide
    * https://aka.ms/asrs/privatelink
    * https://azurecharts.com
    * https://aka.ms/cloud_guide
    * https://aka.ms/appregistrations
    * https://aka.ms/appservicecustomdns
  * Azure SDKs
    * https://azure.github.io/azure-sdk-for-net   
    * https://azure.github.io/azure-sdk-for-python
    * https://azure.github.io/azure-sdk-for-java
    * https://azure.github.io/azure-sdk-for-c
  * Open Service Mesh
    * https://openservicemesh.io
  * Microsoft Azure DevOps
    * https://dev.azure.com
    * https://www.azuredevopslabs.com
    * https://aka.ms/DevOpsLab
    * Terraform in action 
      * https://github.com/terraform-in-action
    * https://aka.ms/cicdhol
  * Microsoft Azure Training 
    * https://aka.ms/azuredevebook
    * http://aka.ms/azure-pluralsight
    * https://www.edx.org/learn/azure
    * https://cloudacademy.com/library/azure
    * http://www.cloudranger.net/azure-training
  * Kubernetes
    * http://aka.ms/k8s/LearnwithPhippy
    * https://aka.ms/k8s
    * https://aka.ms/k8s/lightboard
  * Azure Kubernetes Service   
    * https://aka.ms/k8sonazure
    * https://aka.ms/LearnAKS
    * https://docs.microsoft.com/azure/architecture/reference-architectures/microservices/aks
    * https://docs.microsoft.com/azure/aks/best-practices
    * https://aka.ms/adopt/kubernetes
    * http://aka.ms/aks/roadmap
    * https://aka.ms/aks/previewfeatures
    * https://aka.ms/aks-naming-rules
    * https://aka.ms/kubepolicydoc
    * https://aka.ms/aksbaseline
    * https://aka.ms/aks/containerd
    * https://aka.ms/bridge-to-k8s-vs-quickstart
  * Cloud Adoption Framework
    * https://github.com/topics/cloud-adoption-framework
  * Azure API Management 
    * http://aka.ms/apimpolicyexamples
    * https://feranto.github.io/azure-apim-lab
  * Dev Security tools 
    * https://secdevtools.azurewebsites.net
    * https://aka.ms/mscadocs
  * Azure Synapse Analytics
    * https://aka.ms/synapse
    * https://aka.ms/Mechanics_Synapse
    * https://aka.ms/Synapse_Link_Docs
    * https://web.azuresynapse.net/workspaces
    * Azure Synapse Analytics Toolkit https://azure.microsoft.com/en-us/resources/azure-synapse-analytics-toolkit
          * Four steps to get started with Azure Synapse Analytics https://aka.ms/AA7jf6m 
          * Code-free ETL with Azure Synapse Analytics https://aka.ms/AA7jmts
          * Code-first data analysis with Azure Synapse Analytics https://aka.ms/AA7jf6v
          * Architecture Deep dive Azure Synapse Analytics https://aka.ms/AA7jf7y 
          * Getting started with data tasks using Python in Azure Synapse Analytics https://aka.ms/AA7jmtq
          * Data lake and data warehouse unified with Azure Synapse Analytics https://aka.ms/AA7jmtr
          * The Power BI Professional's Guide to Azure Synapse Analytics https://aka.ms/AA7mx5o
    * Compare price-performance of Azure Synapse Analytics and Google BigQuery https://azure.microsoft.com/en-us/services/synapse-analytics/compare/#price-performance
    * Three Ways Analytics Can Help: Respond, Adapt, and Save https://aka.ms/AA8l97o
    * Azure Synapse SQL architecture https://aka.ms/AA8cio5
    * Cloud Analytics with Microsoft Azure Packt ebook https://aka.ms/AA8annw
  * Azure Cosmos DB
    * https://aka.ms/azurecosmosetcd
    * https://aka.ms/cosmosdb
    * https://aka.ms/cosmosdb-emulator
    * http://aka.ms/cosmosdbworkshop
    * https://aka.ms/cosmos-serverless
    * https://cosmos.azure.com/capacitycalculator
    * Introduction to the Azure Cosmos DB etcd API (preview) https://docs.microsoft.com/en-us/azure/cosmos-db/etcd-api-introduction 
  * Application Patterns in Durable Functions 
    * https://docs.microsoft.com/en-us/azure/azure-functions/durable/durable-functions-overview?tabs=csharp#application-patterns
  * Microsoft Machine Learning for Apache Spark
    * https://aka.ms/spark
  * Azure Databricks operator (for Kubernetes)
    * https://aka.ms/azure-databricks-operator
  * Resilience at work playbook
    * https://www.microsoft.com/en-us/resilience
  * AI for good
    * https://aka.ms/ai-for-good
  * Azure Machine Learning Algorithm Cheat Sheet
    * https://aka.ms/mlcheatsheet
  * Azure IoT
    * https://aka.ms/azureiothub
    * https://aka.ms/iotrefarchitecture
    * https://aka.ms/azure-iot-tools
    * https://github.com/azure/azure-iot-cli-extension
    * https://aka.ms/iot-security-schemas
    * https://aka.ms/iot-workbench
    * https://aka.ms/iot-devkit 
  * Azure IoT Central
    * https://aka.ms/iotcentral
    * https://aka.ms/iotcentral-apps
    * https://aka.ms/iotcentral-sphere-git-readme
    * https://aka.ms/iotcentral-latest-more
  * Continuous patient monitoring app template
    * https://aka.ms/healthtutorial
  * Azure IoT Model Repository 
    * https://aka.ms/iotmodelrepo
  * IoT Workshop
    * https://aka.ms/IoT-online-workshop
  * Azure Sphere 
    * https://docs.microsoft.com/en-us/azure-sphere
  * Deploy Azure IoT Edge on Kubernetes (preview)
    * https://aka.ms/edgek8sdocs
  * Azure IoT Edge Hands-on Labs
    * https://labs.iotedge.dev
  * Live Video Analytics on IoT Edge
    * https://aka.ms/iot_video
  * IoMT FHIR Connector for Azure
     * https://aka.ms/iomt
  * Azure API for FHIR
     * https://aka.ms/fhir
  * FHIR Server Samples
    * https://aka.ms/fhirdemo
  * FHIR Registry
    * https://registry.fhir.org
  * Simplifer.net is a feature rich platform where you find all defining HL7 FHIR resources.
    * https://simplifier.net
  * AN OPEN SOURCE INITIATIVE FOR FHIR BASED FITNESS AND LIFESTYLE RECORDS
    * http://www.fhirdata.exchange
    * https://github.com/fhirfli
  * Pi 4 Bootloader Configuration
    * https://www.raspberrypi.org/documentation/hardware/raspberrypi/bcm2711_bootloader_config.md
    * https://www.raspberrypi.org/forums/viewtopic.php?t=274595
  * ABP Framework - Open Source Web Application Framework for ASP.NET Core
    * https://abp.io
  * Microsoft Adoption
    * https://adoption.microsoft.com
  * Microsoft CSE (Commercial Software Engineering) - Code with engineering playbook
    * https://github.com/microsoft/code-with-engineering-playbook
  * Refactoring.Guru
    * https://refactoring.guru
  * Microsoft and GitHub Open Source 
    * https://opensource.microsoft.com
    * https://opensource.guide
  * Visual Studio App Center
    * https://appcenter.ms/
    * https://aka.ms/appcenter-platform-matrix
  * Development environment on Windows 10
    * https://docs.microsoft.com/windows/dev-environment/overview 
  * GraphQL
    * https://graphql.org/learn
    * https://studio.apollographql.com
    * https://graphql-dotnet.github.io
    * https://github.com/graphql-dotnet/server
  * SharePoint Online and SharePoint Framework (SPFx) toolchain 
    * https://aka.ms/SharePointTrainingPresentation 
    * https://aka.ms/SharePointOnlineTraining
    * https://aka.ms/spfx 
    * https://aka.ms/spfx-webparts
    * https://aka.ms/spfx-extensions
    * https://aka.ms/list-formatting
    * https://github.com/pnp/sp-starter-kit 
    * https://lookbook.microsoft.com
    * https://adoption.microsoft.com/sharepoint
    * http://aka.ms/sppnp-modernize
    * https://aka.ms/PlanningSPHubsites
    * https://aka.ms/MSLists
    * https://aka.ms/sharepoint-bizapps
  * Office Dev Center 
    * https://aka.ms/spdev
    * https://aka.ms/spdev-blog
  * Project Cortex
    * https://aka.ms/ProjectCortex
  * Teams
    * http://aka.ms/SuccessWithTeams
    * https://aka.ms/TeamsToolkit
    * https://aka.ms/teams-samples
    * https://aka.ms/teamsdevices
    * https://aka.ms/teamsvirtualrounding
  * App Studio on Microsoft Teams 
    * https://aka.ms/InstallTeamsAppStudio 
  * Microsoft Teams toolkit 
    * https://aka.ms/teams-toolkit 
  * Teams App Templates
    * https://aka.ms/TeamsAppTemplates
  * Adaptive Cards 
    * https://adaptivecards.io/designer
  * Script Lab 
    * https://aka.ms/getscriptlab
    * https://aka.ms/script-lab
  * Power Platform
    * https://aka.ms/IntroPowerPlatform
    * https://aka.ms/PlatformLimits
    * https://aka.ms/ResponsivePowerApps
    * https://aka.ms/connectors
    * https://www.youtube.com/playlist?list=PLi9EhCY4z99W2QOTgbwhFZEjpqc8YZDVH
    * https://aka.ms/pparchseries
  * Power Apps
    * https://aka.ms/powerapps-resources
    * https://aka.ms/Learn/PowerAppsShapeData
    * https://aka.ms/PaTestEnvironment
    * https://aka.ms/PowerUp
  * Power Apps Studio 
    * https://aka.ms/PowerAppsWin
  * ISV Studio
    * https://isvstudio.powerapps.com
  * Power Automate
    * https://aka.ms/MicrosoftPowerAutomate
    * https://aka.ms/learntoautomate
    * Blog https://aka.ms/powerautomateunattendedrpa
    * RPA https://aka.ms/rpaDesktopGetStarted 
  * Power Virtual Agents
    * https://aka.ms/TryPVA
    * https://aka.ms/PVA/Create
    * Trial https://go.microsoft.com/fwlink/?LinkId=2107702
  * MFA setup
    * https://aka.ms/mfasetup
    * https://mysignins.microsoft.com/security-info
  * Mobile Apps
    * https://aka.ms/PowerAppsiOS
    * https://aka.ms/PowerAppsAndroid
  * Power BI Desktop 
    * https://aka.ms/PBIDesktopStore
  * Microsoft Power BI Embedded Playground
    * https://aka.ms/pbieplayground
  * On-Premises Data Gateway 
    * https://powerapps.microsoft.com/en-us/downloads
    * https://powerbi.microsoft.com/en-us/downloads
  * Microsoft Teams for Linux 
    * https://aka.ms/get-teams-linux 
    * https://aka.ms/teams-linux-docs
  * Power Platform admin center
    * https://aka.ms/ppac or https://admin.powerplatform.microsoft.com
    * https://aka.ms/CoEStarterKit
    * https://aka.ms/powerappscommunityplan
  * Microsoft 365 & Power Platform Blogs
    * https://aka.ms/m365pnp
    * https://aka.ms/m365devyoutube
    * https://aka.ms/m365pnp-videos
    * https://cloudblogs.microsoft.com/powerplatform  
    * https://developer.microsoft.com
    * https://devblogs.microsoft.com
    * https://resources.techcommunity.microsoft.com/demos
    * https://aka.ms/M365DevAccount
  * Coding companion .NET Foundation
    * https://mod-dotnet-bot.net
  * Microsoft 365 DSC
    * https://aka.ms/m365dsc
  * Emergency response solution
    * https://aka.ms/emergency-response-solution
  * Regional Government Emergency Response and Monitoring - Power Platform solution for state and local governments
    * https://aka.ms/rer-docs
  * Crisis Communication sample template in Power Apps
    * https://aka.ms/crisis-communication-app-docs
  * Higher Education Crisis Financial Impact Tracker - Sample solution
    * https://aka.ms/cfit
  * Return to the Workplace solution
    * https://aka.ms/rtw-docs
  * Custom connectors certification
    * https://docs.microsoft.com/en-us/connectors/custom-connectors/submit-certification
    * https://docs.microsoft.com/en-us/connectors/custom-connectors/certification-updates
    * https://powerapps.microsoft.com/en-us/blog/connector-certification-now-in-isv-studio
  * Community 
    * https://techcommunity.microsoft.com 
    * https://powerusers.microsoft.com 
  * Microsoft identity platform
    * https://developer.microsoft.com/en-us/identity
    * https://oauth.net/2
  * OWASP cheatsheet series
    * https://cheatsheetseries.owasp.org
  * Azure on Q&A
    * https://aka.ms/azureqa
  * Fluent UI
    * https://aka.ms/fluentsandbox
    * https://aka.ms/fluentpen
  * Javascript
    * https://javascript.info
  * Anomaly Detector API
    * https://aka.ms/anomaly-detector
  * Speech Service
    * https://aka.ms/csspeech
  * ONNX Runtime
      * https://microsoft.github.io/onnxruntime
  * NNI (Neural Network Intelligence)
    * https://nni.readthedocs.io
  * Playwright
    * https://playwright.dev
  * FASTER
    * https://microsoft.github.io/FASTER
  * Rush Stack
    * https://rushstack.io
  * LearnAI Boot Camp Training Materials
    * https://azure.github.io/LearnAI-Bootcamp
  * ETHICAL AI
    * http://aka.ms/ai-ethics
  * AutoML
    * https://ml.azure.com
    * https://aka.ms/mythbuster-automl
    * https://aka.ms/automl-metrics
    * https://nni.readthedocs.io
   * Curated Learning paths on Developer's Intro to Data Science
     * https://aka.ms/DevIntroDS_Learn
   * Shervine Amidi's illustrated study guides for Data Science tools
     * https://www.mit.edu/~amidi/teaching/data-science-tools
   * Download Mouse Without Borders for Windows 10/8/7
     * https://aka.ms/mm
   * Microsoft Family Safety
     * https://aka.ms/microsoftfamilysafety
   * Microsoft Bug Bounty Program
     * https://aka.ms/bugbounty
   * PatternFly
     * https://www.patternfly.org
   * Swagger Petstore
     * https://redocly.github.io/redoc
   * E-book: Modernize existing .NET Framework applications with Azure and Windows Containers
     * https://aka.ms/liftandshiftwithcontainersebook
   * Sample apps: Modernization of legacy ASP.NET web apps by using Windows Containers
     * https://aka.ms/eshopmodernizing
   * Containerized Docker application lifecycle with Microsoft platform and tools (downloadable e-book)
     * https://aka.ms/dockerlifecycleebook
   * .NET Microservices: Architecture for containerized .NET applications (downloadable e-book)
     * https://aka.ms/microservicesebook
   * Architecting modern web applications with ASP.NET Core and Azure (downloadable ebook)
     * https://aka.ms/webappebook
   * https:///aka.ms/migrateapps
   * https://github.com/iot-for-all
   * https://vnetplanner.chunliu.me
   * https://pycaret.org
   * https://aspnetcore.readthedocs.io
      
[![ForTheBadge makes-people-smile](http://ForTheBadge.com/images/badges/makes-people-smile.svg)](http://ForTheBadge.com)
